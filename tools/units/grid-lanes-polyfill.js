...
function calculateLaneSizes(template, containerSize, gap, fontSize, rootFontSize) {
  ...
  // Local fix: for minmax(min, Nfr) tracks the `fr` must distribute the space
  // left AFTER every track's base (min) size is reserved. The upstream code
  // only subtracted non-fr `fixedSpace`, so it added each fr lane's min on top
  // of a full free-space share and the lanes overflowed. Subtract fr lanes'
  // mins here too so minmax(_, 1fr) sizes correctly.
  let frMinSpace = 0;
  for (const lane of lanes) {
    if (typeof lane.max === "object" && lane.max.fr) frMinSpace += lane.min;
  }
  const flexSpace = Math.max(0, availableSpace - fixedSpace - frMinSpace - totalGaps);
